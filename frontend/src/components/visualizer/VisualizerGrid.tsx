      ...
      {beatMarkers.map((beatIndex) => (
        <div
          ...
          className={cn(
            ...
            beatIndex % (beatCount || 4) === 0 ? 'border-border border-l' : ''
          )}
          style={{
            left: `${beatIndex * beatWidth - (beatIndex == beatCount ? 1 : 0)}px`,
          }}
        />
      ))}
    </div>
  ...
  return (
    ...
    >
      {Array.from({ length: beatCount }).map((_, bIdx) => (
        <div
          key={bIdx}
          className={`font-mono font-bold text-muted-foreground/40 text-center border-border border-l pb-2 last:border-r uppercase ${
            zoomLevel <= 1
              ? 'text-xs'
              : zoomLevel <= 1.5
                ? 'text-sm'
